...
import { getPost, getPostBySlug } from '../../services/blogRepository';
import { getCommentsByPost, postCommentsByPost } from '../../services/commentRepository';
import { isEmptyOrSpaces, useQuery } from '../../utils/utils';
  ...
  useEffect(() => {
    if (post) {
      getCommentsByPost(post.id, ps, p).then((data) => {
        if (data) {
          data.metadata.actionName = `post/${year}/${month}/${day}/${postSlug}`;
